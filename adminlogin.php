...
if(isset($_POST['submit'])){
    ...
    $login = "SELECT * FROM `admin` WHERE `Admin_Email` = '$useremail' AND `Admin_Password` = '$password'";
    $result = $conn->query($login);
    $row = $result->fetch_assoc();

    if($row['Admin_Email'] == $_POST['email'] AND $row['Admin_Password'] == $_POST['password']){
      ...
      $_SESSION['Admin_Name'] = $row['Admin_Name'];
       ...
       ?>
       <script>
        window.alert("Welcome To DashBorad")
...
<div class="form-group">
  ...
  <br>
  <input type="submit" value="Login TO Dashborad" name="submit" class="btn btn-primary px-5 py-3">
</div>
</form>
</div>
